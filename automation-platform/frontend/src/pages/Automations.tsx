  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
        <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-100 p-6 dark:border-slate-800 dark:from-slate-900 dark:to-slate-950">
          <PageHeader
            title="Automations"
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900/50">
          <p className="text-sm font-medium text-slate-900 dark:text-white">Automation usage</p>
          <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-400">
            <div>
              ...
              <div className="mt-1 h-2 rounded-full bg-slate-200 dark:bg-slate-800">
                <div className="h-2 rounded-full bg-cyan-500" style={{ width: `${rows.length ? (rows.filter((row) => row.is_active).length / rows.length) * 100 : 0}%` }} />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/50">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="inline-flex rounded-xl border border-slate-200 bg-slate-100 p-1 text-sm dark:border-slate-800 dark:bg-slate-950/60">
            <button className="rounded-lg bg-white px-3 py-1.5 text-slate-900 dark:bg-slate-800 dark:text-white">Automations</button>
            <Link to={`/w/${workspaceId}/automations/activity`} className="rounded-lg px-3 py-1.5 text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
              Activity log
            </Link>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="secondary">Create folder</Button>
            <Link to={`/w/${workspaceId}/automations/new/builder`} className="inline-flex items-center justify-center rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white hover:bg-cyan-500">
      ...
      <form onSubmit={add} className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/40">
        ...
        <input
          ...
          className="w-full max-w-xs rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
        />
        ...
        <textarea
          ...
          rows={18}
          className="w-full rounded-lg border border-slate-300 bg-white p-3 font-mono text-xs text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
        />
        <Button type="submit">
          Create automation
        </Button>
      ...
      {testAutomation ? (
        <section className="space-y-4 rounded-2xl border border-cyan-500/30 bg-white p-4 shadow-xl shadow-slate-200/30 dark:bg-slate-900/80 dark:shadow-cyan-950/10">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-white">Test automation: {testAutomation.name}</p>
              <p className="mt-1 text-sm text-slate-400">
                Run immediately, or arm the automation so the next message from this contact becomes the test trigger.
              </p>
            </div>
            <button type="button" onClick={() => setTestAutomation(null)} className="text-sm text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
              Close
            </button>
          </div>
          <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <label className="block text-sm">
              <span className="mb-1 block font-medium text-slate-700 dark:text-slate-300">Find contact</span>
              <TextInput
                value={contactSearch}
            ...
            <label className="block text-sm">
              <span className="mb-1 block font-medium text-slate-700 dark:text-slate-300">Contact</span>
              <select
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {rows.filter((row) => row.name.toLowerCase().includes(search.toLowerCase())).map((r) => (
              <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                ...
                <td className="px-4 py-3 text-slate-600 dark:text-slate-400">Workspace owner</td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-400">Recently</td>
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs ${r.is_active ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-amber-500/15 text-amber-700 dark:text-amber-300'}`}>
                    {r.is_active ? 'Active' : 'Paused'}
                  </span>
                </td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{r.trigger_type ?? 'message.received'}</td>
                ...
                <td className="px-4 py-3 text-right">
                  ...
                  <Link to={`/w/${workspaceId}/automations/${r.id}/builder`} className="ml-3 text-sm text-cyan-600 hover:text-cyan-500 dark:text-cyan-300 dark:hover:text-cyan-200">
                    Builder
                  </Link>
                  <button
                    type="button"
